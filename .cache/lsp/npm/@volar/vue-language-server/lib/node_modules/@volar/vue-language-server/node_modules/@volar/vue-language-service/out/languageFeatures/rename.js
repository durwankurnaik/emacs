                    ...
                    if (workspaceEdit.changes) {
                        for (const editUri in workspaceEdit.changes) {
                            ...
                            for (const textEdit of textEdits) {
                                ...
                                if (teleport) {
                                    for (const [teleRange] of teleport.findTeleports(textEdit.range.start, textEdit.range.end, sideData => !!sideData.capabilities.rename)) {
                                        if (recursiveChecker.has({ uri: teleport.document.uri, range: { start: teleRange.start, end: teleRange.start } }))
                                            continue;
                                        foundTeleport = true;
                                        yield withTeleports(teleport.document, teleRange.start, newName);
                                    }
                                }
                                if (!foundTeleport) {
                                    if (!result.changes)
            ...
            if (vscode.TextDocumentEdit.is(tsDocEdit)) {
                ...
                if (sourceMap) {
                    ...
                    for (const tsEdit of tsDocEdit.edits) {
                        for (const [vueRange] of sourceMap.getSourceRanges(tsEdit.range.start, tsEdit.range.end, data => typeof data.capabilities.rename === 'object' ? data.capabilities.rename.out : !!data.capabilities.rename)) {
