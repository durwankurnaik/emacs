...
function entriesToLocations(rootUri, entries, getTextDocument) {
    const locations = [];
...
function entriesToLocationLinks(rootUri, entries, getTextDocument) {
    const locations = [];
    for (const entry of entries) {
        const entryUri = shared.getUriByPath(rootUri, entry.fileName);
        const doc = getTextDocument(entryUri);
...
function boundSpanToLocationLinks(rootUri, info, originalDoc, getTextDocument) {
    const locations = [];
    if (!info.definitions)
        return locations;
